/* session.js - sessions module for node.js
   ...
   Change History:  
     2017-02-11: changed by Gordon Chen
     MIT License
...
function lookupOrCreate(req,opts){var id,session
  ...
  session.cookie.expiration=(+new Date)+session.cookie.lifetime*1000
...
function Session(id,opts){
  ...
  if(opts.lifetime){
    cookie.persistent = 'persistent' in opts ? opts.persistent : true
    cookie.lifetime=opts.lifetime
  }
  else{
...
Session.prototype.getSetCookieHeaderValue=function(){var parts
  parts=['SID='+this.cookie.id]
  if(this.cookie.path) parts.push('path='+this.cookie.path)
  if(this.cookie.domain) parts.push('domain='+this.cookie.domain)
  if(this.cookie.persistent) parts.push('expires='+dateCookieString(this.cookie.expiration))
  return parts.join('; ')}
...
Session.prototype.destroy=function(){
  delete sessions[this.cookie.id]}
